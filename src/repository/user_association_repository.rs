...
pub async fn batch_select_user_associations(openid: &str) -> Result<Vec<UserAssociationDTO>> {
    Ok(sqlx::query_as!(
        UserAssociationDTO,
...
pub async fn select_user_associations_by_idp_openid(
    ...
    Ok((
        ...
        batch_select_user_associations(&idp_association.openid).await?,
    ))
}

pub async fn batch_create_association(
    openid: &str,
    associations: &mut Vec<UserAssociationDTO>,
) -> Result<()> {
    let mut builder: QueryBuilder<'_, sqlx::MySql> =
        QueryBuilder::new("INSERT INTO t_user_association(openid, idp_type, idp_openid) ");
